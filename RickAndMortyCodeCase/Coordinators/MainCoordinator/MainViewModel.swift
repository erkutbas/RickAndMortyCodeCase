...
class MainViewModel: BaseViewModelDelegate {
    ...
    func getData(with pagination: Bool = false) {
        ...
        characterListUseCase.execute(useCaseCallBack: callBack, params: mainViewDataFormatter.getRickAndMortyCharacterRequest())
    ...
    // MARK: - Listeners
    private func listenerHandler(with result: Result<RickAndMortyCharacterResponse, ErrorResponse>) {
    ...
    private lazy var datalistener: (Result<RickAndMortyCharacterResponse, ErrorResponse>) -> Void = { [weak self] result in
